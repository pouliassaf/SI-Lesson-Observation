    ...
    if page == strings["page_lesson_input"]:
        ...
        if selected_option == strings["option_create_new"]:
            ...
            keys_to_reset = ['observer_name', 'teacher_name', 'teacher_email', 'operator', 'school_name', 'grade',
                             ...
                             'observation_date', 'period', 'observation_type', 'overall_notes', 'checkbox_send_feedback', 'auth_email_input']

            for key in keys_to_reset:
                 # Keep the auth_email_input if it was successfully entered
                 if key == 'auth_email_input' and st.session_state.get('auth_email_input'):
                      continue
            ...
            try:
                 ...
                 for key, value in existing_data.items():
                     ...
                     elif value is not None and value != "": # For other basic info, load if not None or empty
                         st.session_state[key] = value
                     elif key not in st.session_state: # If key doesn't exist in state yet, set it (might be None)
                         st.session_state[key] = value
